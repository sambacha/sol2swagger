...
module.exports = {
  ...
  compileFromAbi: function ($abi, $name, $bytecode) {
    ...
    var functions = $abi.filter(function (item) {
      return item.type === 'function';
    });
    var events = $abi.filter(function (item) {
      return item.type === 'event';
    });
    var paths = this.pathsTemplate('get', functions, $name).paths;
    if ($bytecode && $name) {
      paths['/' + $name + '/deploy'] = this.pathTemplate(
        'post',
        this.deployTemplate($name),
        $name,
        $bytecode
      )['/' + $name + '/deploy'];
    }
  ...
  propertyTemplate: function ($property) {
    ...
    if ($property.type === undefined || isArray || Object.keys($property).length > 3)
      throw 'Object provided is not a property';
  ...
  definitionsTemplate: function ($operations) {
    ...
    Object.keys($operations).forEach(function (key) {
      var nameAdjust = '';
      if ($operations[key].type === 'event') {
        nameAdjust = 'Event';
      }
      definitions[$operations[key].name + nameAdjust] = module.exports.definitionTemplate(
        $operations[key]
      );
