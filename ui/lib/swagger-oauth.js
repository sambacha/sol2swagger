    ...
    if (scope.description) {
      if (
        ...
        str +=
          ...
          ')' +
          '</span>';
      else str += '<br/><span class="api-scope-desc">' + scope.description + '</span>';
    }
    str += '</label></li>';
    popup.append(str);
  }
    ...
    for (var key in authSchemes) {
      if (authSchemes.hasOwnProperty(key) && OAuthSchemeKeys.indexOf(key) != -1) {
        ...
        if (
          ...
          url =
            dets.authorizationUrl + '?response_type=' + (flow === 'implicit' ? 'token' : 'code');
    ...
    if (token.error) {
      ...
      if (b) {
        ...
        window.swaggerUi.api.clientAuthorizations.add(
          ...
          new SwaggerClient.ApiKeyAuthorization('Authorization', 'Bearer ' + b, 'header')
        );
